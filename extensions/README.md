...
Files here are full tools with their own panel — Furni Hider, Room Clone, Area Mover,
Room History, Marktplaats Alerts, etc. For a one-off script with no UI, use the in-game
Extensions panel instead (paste JS directly into the hotel client — see the root
`EXTENSIONS.md`, a separate system).

There's no auto-registering tile hub. A file here only gets an entry point once you wire
a row for it into the main Gheloo hub in `content.js`.
